...
export type PaymentMethodType = 'CREDIT' | 'DEBIT' | 'CASH' | null
...
export const CartContextProvider = ({ children }: ICartContextProvider) => {
  ...
  const [cartState, dispatch] = useReducer(cartReducer, {
    itemsInCart: storedCartAsJSON ? JSON.parse(storedCartAsJSON) : [],
    paymentMethod: null,
    showPaymentNotSelectedError: false
  })

  const { itemsInCart, paymentMethod } = cartState

  function setPayment(method: PaymentMethodType) {
    dispatch({ type: CartActions.SET_PAYMENT_METHOD, payload: { method } })
  }
  ...
  function addItemToCart(id: number, amount: number) {
    dispatch({ type: CartActions.ADD_ITEM_TO_CART, payload: { amount, itemId: id } })
    toast.success('Item adicionado ao carrinho')
  }

  function removeOneItemFromCart(id: number) {
    dispatch({ type: CartActions.REMOVE_ONE_ITEM_FROM_CART, payload: { itemId: id } })
